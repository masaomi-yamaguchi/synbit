...
module Synthesis.LazyEvalPTrace where
...
evalPTrace_ env start v (MRConstL _ e) MTrConstL = do
  ...
  if b
    then S.empty
      ...
      mzero
evalPTrace_ env start v (MRCaseB _ e0 balts oenv) (MTrBranch _ tr0 chose tr) =
  do
  ...
  where
    evalBAltsP :: (MonadPlus m, Sharing m, MonadFail m)
               ...
               -> MTrace m
               -> m (Store m)
    evalBAltsP env start v e0 v0 obalts mbalts oenv (MTrBranch _ tr0 mchose tr) = do
      ...
      if exit'
        then do
          ...
          (store, sw') <- doReconcile env start v e0 tr0 v0 obalts oenv tr
          chose <- mchose
          guard (chose == sw')
          store
    ...
    doReconcile
      ...
      -> m (MTrace m)
      -> m (m (Store m), Int)
    doReconcile env start v e0 tr0 v0 obalts oenv tr = do
      ((p, g, mb), heads, sw') <- findMatchSW 0 v nil =<< obalts
      heads <- share heads
      b <- mb
        ...
        Just bind -> do
          ...
          if g''
            then do
              body <- mrBody b
              return
                (putE0 env bind start v e0 tr0 oenv p g heads body tr, sw')
            else do
    ...
    findMatchSW
      ...
      -> List m (MRBAlt m)
      -> m (MRBAlt m, m (List m (MRBAlt m)), Int)
    findMatchSW _ _ heads Nil = mzero -- よいswitch先が見つからない
    findMatchSW i v heads (Cons mpgb rest) = do
      (p, g, b) <- mpgb
      exit <- mrExitCond =<< b -- すでにshareされている
      exit' <- convertRB $ appR exit v
      if exit'
        then return ((p, g, b), heads, i)
        else findMatchSW (i + 1) v (cons (return (p, g, b)) heads) =<< rest
